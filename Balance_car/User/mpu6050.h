...
/* ADO引脚接不接 */
#if	defined	MPU6050_ADO_JOIN_NO

	//AD0接VCC,IIC地址为0X69
	#define		_MPU_ADDRESS									0x68			//mpu6050_i2c地址,  W: 0x69<<1 -->> 0xD2, R: 0x69<<1 + 1 -->> 0xD3

	//AD0接VCC
	#define		_MPU_WRITE										0xD0			//mpu6050地址, I2C设备7位地址值,HAL库用8位整字节写入，所以在数据表中必须在调用接口前左移一位 (( 0xD2>>1 )=0x69)
	#define		_MPU_READ											0xD0			
...
{
		//原始六轴数据
		float mpu_acce[3];				//加速度		0x3B
		float mpu_gyro[3];				//陀螺仪		0x43
