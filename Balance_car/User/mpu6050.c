...
//	HAL_Delay(50);
	...
	w_mpu_set_sample_rate(0x13);//50HZ
...
//		return mpu_busy;								//iic忙碌（0x02/0x2）
	...
	//整合MPU原始数据
	mpu_raw_msg_buff->mpu_acce[0] 			= (int16_t)(msg_buff[0]<<8	| msg_buff[1]);		//X轴加速度值
	mpu_raw_msg_buff->mpu_acce[1]				= (int16_t)(msg_buff[2]<<8	| msg_buff[3]);		//Y轴加速度值
	mpu_raw_msg_buff->mpu_acce[2]				= (int16_t)(msg_buff[4]<<8	| msg_buff[5]);		//Z轴加速度值
	mpu_raw_msg_buff->mpu_acce[0]=mpu_raw_msg_buff->mpu_acce[0]/16384.0;
	mpu_raw_msg_buff->mpu_acce[1]=mpu_raw_msg_buff->mpu_acce[1]/16384.0;
	mpu_raw_msg_buff->mpu_acce[2]=mpu_raw_msg_buff->mpu_acce[2]/16384.0;
...
mpu_state_t w_mpu_write_byte(uint8_t reg_addr, uint8_t w_data)
{
	...
	if( HAL_I2C_Mem_Write(&hi2c2, _MPU_WRITE, reg_addr, I2C_MEMADD_SIZE_8BIT, &w_data_byte, 1, 0xfff) == HAL_OK)
		return mpu_ok;					//写入成功
	else
		return mpu_err;					//写入失败
...
uint8_t w_mpu_read_byte(uint8_t reg_addr)
{
	...
	if( HAL_I2C_Mem_Read(&hi2c2, _MPU_READ, reg_addr, I2C_MEMADD_SIZE_8BIT, &r_data_byte, 1, 0xfff) == HAL_OK)
//	HAL_Delay(50);
...
mpu_state_t w_mpu_write_byte_len(uint8_t reg_addr, uint8_t *w_data_buf, uint8_t len)
{
	...
	if( HAL_I2C_Mem_Write(&hi2c2, _MPU_WRITE, reg_addr, I2C_MEMADD_SIZE_8BIT, w_data_buf, len, 0xfff) == HAL_OK)
		return mpu_ok;					//写入成功
	else
		return mpu_err;					//写入失败
...
mpu_state_t w_mpu_read_byte_len(uint8_t reg_addr, uint8_t *r_data_buf, uint8_t len)
{
	...
	if( HAL_I2C_Mem_Read(&hi2c2, _MPU_READ, reg_addr, I2C_MEMADD_SIZE_8BIT, r_data_buf, len, 0xfff) == HAL_OK)
//		HAL_Delay(50);
		return mpu_ok;					//读取完成
	else
...
{
	//超时时间看实际更改, 问题不大
	return HAL_I2C_Mem_Write(&hi2c2, slave_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, w_data, len, 500);
}
...
{
	//超时时间看实际更改, 问题不大
	return HAL_I2C_Mem_Read(&hi2c2, slave_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, r_data, len, 1000);
}
